...
def analyze_real_kolam(image_path, output_dir="public/results"):
    ...
    try:
        ...
        # Save analysis.txt with base name
        analysis_txt_path = os.path.join(output_dir, f"{base_name}_analysis.txt")
        with open(analysis_txt_path, "w", encoding="utf-8") as f:
            f.write("KOLAM ANALYSIS RESULTS\n")
        # Append detailed results to the new analysis.txt
        print_detailed_results(result, output_dir, analysis_txt_path)

        # Create visualization with base name
...
def print_detailed_results(result, output_dir, analysis_txt_path):
    """Print comprehensive analysis results and append to analysis_txt_path"""
    print("\n" + "=" * 60)
    ...
    with open(analysis_txt_path, "a", encoding="utf-8") as f:
